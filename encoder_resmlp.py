...
class MLP(torch.nn.Module):
    def __init__(self, dims, init=0.25, last_relu=False, bn=True, last_bn=True):
...
class ResBlock(torch.nn.Module):
    def __init__(self, dim, mdim=None):
        super().__init__()
        if mdim is None:
            mdim = dim
        self.fc1 = nn.Linear(dim, mdim)
        self.bn1 = nn.BatchNorm1d(mdim)
        self.relu1 = nn.ReLU()
        self.fc2 = nn.Linear(mdim, dim)
    ...
    def forward(self, x):
        ...
        return self.relu(o + x)

class ResMLP(nn.ModuleList):
    def __init__(self, dim, num_layers=1, mdim=None):
        super().__init__()
        self.blo = nn.ModuleList([ResBlock(dim, mdim=mdim) for _ in range(num_layers)])
...
class EncoderLayer(torch.nn.Module):
    ...
    def __init__(self, ind, layer_type, idim, odim, sdim=None, dropout=0.5, relu_weight=0.005, srate=0.25, dense_mlp=False):
        ...
        if layer_type == 'leaf':
            if idim == 3:
                ...
                self.pts_align = lambda x : x
                #self.bn = nn.BatchNorm1d(idim)
                # self.pts_align = lambda x : self.bn(x.transpose(-1, -2)).transpose(-1, -2)
            
            self.mlp = MLP([idim, 1024, odim], init=relu_weight)
            self.extract = ResMLP(odim)
            ...
            if idim != odim:
                ...
                self.extract = torch.nn.Identity()
    ...
    def forward(self, ans, sample=None, vec=None, dmap=None, drev=None):
        if self.layer_type[0] == 'l':
            ...
            ans = self.upload(ans[:, self.child_lr])
            ans = self.extract(ans)
            ans = ans.reshape(ans.size(0), 2, -1, self.odim).max(dim=1)[0]
...
class Encoder(torch.nn.Module):
    ...
    def __init__(self, N, sample_layers, dim, OUTPUT, point_dim=3, dim_layer0=16, dim_repeat_cut=4, rotate=True, use_symmetry_loss=False, channel=1, sample_child_first=True):
        ...
        self.tree = build_tree.BuildTree(N, sample_layers, sample_child_first=sample_child_first, use_symmetry_loss=use_symmetry_loss)
        ...
        for i in reversed(range(self.num_layers)):
            ...
            if layer_type != 'leaf':
                idim = feature_dim
                if odims[-dim_repeat] in ([feature_dim] if dim_repeat_cut != 0 else [feature_dim]):
