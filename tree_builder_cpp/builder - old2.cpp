...
namespace builder_cpp {
	...
	#define GRID_DEF 0.05
	#define MAX_DIFF_LIM_DEF 2
	#define SYM_THRES 0.75
...
// fprintf(stderr, "d = %.4lf %.4lf %.4lf\n", d[0], d[1], d[2]);
		...
		bool shrink_symmetry(vector<P>& pts_ref, int d, bool apply_shrink=true, float grid_coef=GRID_DEF, 
			...
			int good = 0;
			auto pos = [grid] (const P& p) -> Triple 
				{return (Triple) {int(p.d[0] / grid), int(p.d[1] / grid), int(p.d[2] / grid)}; };
...
if (good >= n * thres) {
	...
	fflush(stderr);
		...
		vector<P> build(vector<P> p, vector<P> fixed_part_v=vector<P>(), bool fixed_flipped=false) {
			int n = len(p);
			if (!init) {
				init = true;
				p4build = p;
...
#ifdef NOBIND
fprintf(stderr, "build n = %d leaf_size = %d mode = symmetric v = (%.4lf, %.4lf, %.4lf)\n", 
...
// 	debug_print_pts(shrink_pts);
							...
							part_v += build(vector<P>(p.begin(), p.begin() + n / 2),	fixed, 1);
							part_v += build(vector<P>(p.begin() + n / 2, p.end()),		fixed, 0);
