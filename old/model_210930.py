...
class FC(torch.nn.Module):
    ...
    def forward(self, input):
        return self.activate(self.linear(input))
...
class EncoderLayer(torch.nn.Module):
    ...
    def __init__(self, layer_type, channel, dim, dropout=0.5, relu_weight=0.25, input_dim=0, sample_dim=0):
        ...
        if layer_type == 'leaf':
            ...
            self.merge = nn.ModuleList([nn.Linear(input_dim * 2, dim) for _ in range(ndir)])
            self.merge_activate = nn.PReLU(init=relu_weight)
            
            if layer_type == 'sampled':
                self.maintain_sample_channels = FC(channel * sample_dim, channel * dim, init=relu_weight)
                self.merge_sample = FC(dim * 2, dim, init=relu_weight)
    ...
    def forward(self, last, sample=None, vec=None, dmap=None, drev=None):
        if self.layer_type[0] == 'l':
            ...
            ans = torch.cat([
                    lch * (1 - drev)[:, :, None, None] + rch * drev[:, :, None, None], 
                    rch * (1 - drev)[:, :, None, None] + lch * drev[:, :, None, None]
                ], dim=-1)
            # (batch_size, layer_size, channel, input_dim*2)

            vec = dmap[vec]
            tmp = torch.empty(*ans.shape[:-1], dim).cuda()
            for i, mer in enumerate(self.merge):
                flag = (vec == i)
                tmp[flag] = mer(ans[flag])
            
            ans = self.merge_activate(tmp)
            ...
            if self.layer_type[0] == 's':
                ...
                sample = self.maintain_sample_channels(sample)
                sample = sample.view(*sample.shape[:-1], channel, -1)
                
                ans = torch.cat([ans, sample], dim=-1)
                # (batch_size, layer_size, channel, dim * 2)
