...
class Vector(torch.nn.Module):
	...
	def forward(self):
		if self.fixed is None:
			...
			x = torch.sin(theta) * sin_phi
			y = torch.cos(theta) * sin_phi
			z = torch.cos(phi)
		else:
			u = self.u.fmod(1)
			# assert not u.isnan().item()
...
class BuildTree:
	...
	def structure(self):
		...
		mem = []
		layers = [[] for i in range(len(bin(N)))]
