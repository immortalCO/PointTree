...
class BuildTree:
    ...
    def __init__(self, N, sample_layers, sample_child_first=True, sample_cross=False, use_symmetry_loss=False):
        while (N & -N) != N:
            ++N
        self.N = N
    ...
    def arrange_pca(self, pts):
        ...
        def build(ind, dep, fixed=None):
            ...
            if self.use_symmetry_loss and n > 8: # and dep <= 3:
                assert (n >> self.sample_layers) == 0, "symmetry loss not captible with sample layers"
                dirs = self.dir_symloss

                best = -0.20 + 1e-5
                # print(f"symloss #{dep}")
                for v in dirs:
                    loss, _dotprod = symmetry_loss(pts[ind], v)
                    if loss < best:
                        best = loss
                        vec = v
                        dotprod = _dotprod

                    # print(f"\tv = {v} loss = {loss}")

                # if dep <= 5:
                #     print(f"vec = {vec} best = {best}")
    ...
    def arrange(self, pts, pca=True, basic=False, debug=False, rotate=True, rotate_only=False, extra=False, num_rotate=10, vec_per_point=4, device='cpu'):
        ...
        if rotate:
            ...
            for i_rotate in range(num_rotate):
                ...
                flag_z = False
                flag_z = flag_z or (z_axis - torch.tensor([0, 0, 1])).norm() < 1e-6
                flag_z = flag_z or (z_axis - torch.tensor([0, 0, -1])).norm() < 1e-6
                ...
                if pca:
                    x_axis = V.T[1]
                    x_axis /= x_axis.norm()
                    x_axis = rotate_towards(x_axis, z_axis)
                    debug_print("x_axis z rotated", format(x_axis))
