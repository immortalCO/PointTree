...
class BuildTree:
    ...
    def arrange_cpp(self, pts, complete_output=False):
        ...
        with open(f"/tmp/points.txt", 'w') as file:
            for p in pts.cpu().numpy().tolist():
                file.write("%lf %lf %lf\n" % tuple(p))

        system(f"/tmp/build_tree {self.sample_layers} arrange {self.N} < /tmp/points.txt > /tmp/arrange.txt")

        arrange = []
        with open(f"/tmp/arrange.txt") as file:
            for line in file:
                if len(line) == 0:
                    continue
                arrange.append(int(line))

        assert len(arrange) == self.layer_size[-1]
        arrange = torch.tensor(arrange).cuda()

        if complete_output:
            output = list(reversed([torch.ones(s) * (i % 3) for i, s in enumerate(self.layer_size)]))
        else:
            output = list(reversed([torch.tensor(i % 3) for i, s in enumerate(self.layer_size)]))
        output[0] = arrange

        return output, arrange
