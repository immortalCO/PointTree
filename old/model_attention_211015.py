...
class EncoderLayer(torch.nn.Module):
    ...
    def __init__(self, layer_type, channel, dim, dropout=0.5, relu_weight=0.25, input_dim=0, sample_dim=0):
        ...
        if layer_type == 'leaf':
            ...
            # self.upload = FC(channel * input_dim, channel * dim, init=relu_weight, flatten=(channel, dim))
            self.attl = Attention(input_dim, input_dim, embed=dim)
            self.attr = Attention(input_dim, input_dim, embed=dim)
            # self.merge = FC(dim * 4, dim, init=relu_weight)
            self.merge = nn.ModuleList([FC(input_dim * 4, dim, init=relu_weight) for _ in build_tree.get_directions()])
            
            if layer_type == 'sampled':
                self.upload_sample = FC(channel * sample_dim, channel * dim, init=relu_weight, flatten=(channel, dim))
                self.attans = Attention(dim, dim)
                self.attsmp = Attention(dim, dim)
    ...
    def forward(self, last, sample=None, vec=None, dmap=None, drev=None):
        if self.layer_type[0] == 'l':
            ...
            drev = drev[vec].long()
            nlch = lch * (1 - drev)[:, :, None, None] + rch * drev[:, :, None, None]
            nrch = rch * (1 - drev)[:, :, None, None] + lch * drev[:, :, None, None]
            
            lch = nlch
            rch = nrch

            def merge_attn(lch, rch, attl, attr, merge):
                return merge(torch.cat([
                        lch, attl(lch, rch, rch),
                        rch, attr(rch, lch, lch),
                    ], dim=-1))

            # ans = merge_attn(lch, rch, self.attl, self.attr, self.merge)
            ans = merge_attn(lch, rch, self.attl, self.attr, lambda x : x)

            vec = dmap[vec]
            tmp = torch.empty(*ans.shape[:-1], self.dim).cuda()
            ...
            if self.layer_type[0] == 's':
                ...
                ans = merge_attn(ans, smp, self.attans, self.attsmp, self.merge_sample)
...
class Encoder(torch.nn.Module):
    ...
    def __init__(self, N, sample_layers, dim, OUTPUT, channel=1, rotate=True):
        ...
        dim_repeat = 2
        cur_dim = 8
        dims = [cur_dim] * (self.sample_layers * dim_repeat - 1)

        relu_weight = 0.32
        relu_weight_dec = 0.01
        relu_weight += relu_weight_dec
        ...
        for i in reversed(range(self.num_layers)):
            ...
            if layer_type != 'leaf':
                mod = len(dims) % dim_repeat
                if mod == 0:
                    cur_dim = min(dim, dims[-dim_repeat] << 1)
                # else:
                #     fact =  mod / dim_repeat
                #     cur_dim = min(dim, int(dims[-mod] * (2 ** fact) + 0.5))
            ...
            logging.info(f"layer {i} ({layer_type}) # = {layer.num_nodes} dim = {layer.dim} relu_weight = {relu_weight}")
