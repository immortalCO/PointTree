...
class BuildTree:
    ...
    def arrange_pca(self, pts):
        ...
        def build(ind, fixed=None):
            n = len(ind)

            if n == 1:
                arrange.append(ind.item())
                return
            ...
            cho_ind2 = dotprod.topk(k=n // 2, largest=False, sorted=False).indices
            ...
            if (n >> self.sample_layers) > 0:
                s = n >> self.sample_layers
                cho = torch.randperm(n)[:s]
                build(ind[cho], vec)


        build(torch.arange(len(pts)))
    ...
    def arrange(self, pts, pca=False, basic=False, debug=False, rotate=True, rotate_only=False, extra=False, num_rotate=10, vec_per_point=4, device='cpu'):
        ...
        if rotate:
            ...
            for i_rotate in range(num_rotate):
                ...
                flag_z = (z_axis - torch.tensor([0, 0, 1])).norm() < 1e-6
